...
lr_rate = 0.001
with open('log_classification.txt', 'w') as logfile: 
...
with tf.device('/gpu:0'):
    ...
    # print(np.shape(train_it))
    print('Batch shape=%s, min=%.3f, max=%.3f' % (batchX.shape, batchX.min(), batchX.max()))

    model_3DCNN = Sequential()

    model_3DCNN.add(Reshape((1350,500, 10,1)))
    model_3DCNN.add(Convolution3D(8, (1, 1,1), activation='relu', input_shape=(1350, 500, 10,1), padding = 'same'))
    model_3DCNN.add(MaxPooling3D((3, 3,3)))
    model_3DCNN.add(Dropout(0.3))
    BatchNormalization()

    model_3DCNN.add(Flatten())
    model_3DCNN.add(Dense(64, activation='relu'))
    model_3DCNN.add(Dense(6, activation = 'sigmoid'))
    ...
    history = model_3DCNN.fit_generator(train_it, epochs=nb_epoch, steps_per_epoch=1013//batchsize,
                    validation_data=valid_it, validation_steps=287//batchsize, class_weight = class_weights)

    model_3DCNN.save('model_3DCNN_augmented_10h39_Thurs_all.h5')
